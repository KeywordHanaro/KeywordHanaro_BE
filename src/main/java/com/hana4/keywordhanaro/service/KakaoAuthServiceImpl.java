...
public class KakaoAuthServiceImpl implements KakaoAuthService {
	...
	@Value("${KAKAO_MULTI_REDIRECT_URI}")
	private String multiRedirectUri;
	...
	@Override
	public String getAccessTokenMulti(String code) {
		// HTTP 헤더 설정
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
		...
		body.add("grant_type", "authorization_code");
		body.add("client_id", clientId);
		body.add("redirect_uri", multiRedirectUri);
		body.add("code", code);
		body.add("client_secret", clientSecret);
		if (clientSecret != null && !clientSecret.isEmpty()) {
			body.add("client_secret", clientSecret);
	...
	@Override
	public void sendMessage(String accessToken, SettlementReqDto settlement) {
		List<UserDto> groupMember = settlement.getGroupMember();
		BigDecimal amount = settlement.getAmount();
		AccountDto account = settlement.getAccount();
		...
		String response = restTemplate.exchange(KAKAO_FRIENDS_MESSAGE_SEND_URL, HttpMethod.POST, request, String.class)
			.getBody();
	}

	@Override
	public void sendMultiMessage(String accessToken, List<SettlementReqDto> settlementList) {
		for (short i = 0; i < settlementList.size(); i++) {
			SettlementReqDto settlement = settlementList.get(i);
			List<UserDto> groupMember = settlement.getGroupMember();
			BigDecimal amount = settlement.getAmount();
			AccountDto account = settlement.getAccount();
			String type = settlement.getType();
			...
			String response = restTemplate.exchange(KAKAO_FRIENDS_MESSAGE_SEND_URL, HttpMethod.POST, request,
					String.class)
				.getBody();
		}
	}
